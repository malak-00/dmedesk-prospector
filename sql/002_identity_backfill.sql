-- DME Desk Prospector: one-time identity grouping backfill.
-- MANUAL ONLY. Run after 001_identity_schema.sql.
--
-- This uses the provider columns already referenced by the repository's
-- duplicate-audit SQL. Review the source schema and row counts first.
...
select
  ...
  phone,
  case
    when name_key <> '' and state_key <> '' and official_key <> '' and phone_key <> ''
      then 'strict:' || name_key || ':' || state_key || ':' || official_key || ':' || phone_key
    else 'singleton:' || npi
  end as identity_key,
  case
    when name_key <> '' and state_key <> '' and official_key <> '' and phone_key <> '' then 'strict'
    else 'singleton'
  end as grouping_tier,
  nullif(name_key, '') as name_key,
  nullif(state_key, '') as state_key,
  nullif(official_key, '') as official_key,
  nullif(phone_key, '') as phone_key
from normalized;
...
on conflict (npi) do update set
  ...
  evidence = public.lead_group_members.evidence || excluded.evidence;
