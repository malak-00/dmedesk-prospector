// Port of appscript/services/AiBriefService.js -- same endpoint/header
// pattern (x-goog-api-key), UrlFetchApp -> fetch.
...
function buildPrompt(company) {
  const dm = company.decisionMakers && company.decisionMakers[0];
  const dmLine = dm
  ...
  if (typeof places.rating === "number") {
    reputationLine =
      "Online reputation: rated " + places.rating + "/10 from " + (places.ratingCount != null ? places.ratingCount : "an unknown number of") + " reviews on Foursquare.";
  ...
  if (company.medicare && company.medicare.totalClaims != null) {
    medicareLine =
      "Medicare DMEPOS volume (CMS public data): " +
      company.medicare.totalClaims +
      " claims" +
...
export async function generateCallBrief(config, company) {
